...
namespace Capture
{
	...
	{
		public:
			virtual bool Send(const void *buffer, UInt32 size) = 0;
	};

	class SocketOutStream : public OutStream
	{
		public:
			SocketOutStream(Socket &s);

			virtual bool Send(const void *buffer, UInt32 size);
	...
	class AsyncStream
	{
		...
		public:
			...
			static bool         FlushAll(OutStream &outStream);
		...
		private:
			...
			bool Flush(OutStream &outStream);
