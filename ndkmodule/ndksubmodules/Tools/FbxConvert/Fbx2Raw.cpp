...
static void ReadMesh( FbxManager * pFbxManager, FbxScene * pScene, FbxNode * pNode, const Hash< const FbxTexture *, FbxString > & textureNames, RawModel & raw )
{
	...
	for ( int polygonIndex = 0; polygonIndex < pMesh->GetPolygonCount(); polygonIndex++ )
	{
		...
		// If there is no diffuse try using the emissive as the diffuse.
		if ( textures[RAW_TEXTURE_USAGE_DIFFUSE] == -1 )
		{
			textures[RAW_TEXTURE_USAGE_DIFFUSE] = textures[RAW_TEXTURE_USAGE_EMISSIVE];
			textures[RAW_TEXTURE_USAGE_EMISSIVE] = -1;
		}
...
static FbxString GetFileFolder( const char * fileName )
{
	...
	{
		folder = String( FileUtils::GetCurrentFolder() + folder.Buffer() ).ToCStr();
	}
	return folder;
}
...
static String GetInferredFileName( const char * fbxFileName, const char * directory, const Array< String > & directoryFileList )
{
	...
	const String fileName = StringUtils::GetFileNameString( StringUtils::GetCleanPathString( fbxFileName ).ToCStr() );
	...
	const String fileBase = isDDS ? fileName.Substring( 0, fileName.GetLength() - 6 ) : StringUtils::GetFileBaseString( fileName.ToCStr() );

	// Try to find a match without file extension.
	for ( int i = 0; i < directoryFileList.GetSizeI(); i++ )
	{
		const String listedFileBase = StringUtils::GetFileBaseString( directoryFileList[i].ToCStr() );
...
static void FindFbxTextures( FbxScene * pScene, const char * fbxFileName, const char * extensions, Hash< const FbxTexture *, FbxString > & textureNames )
{
	...
	const FbxString fbmFolderName = folder + StringUtils::GetFileBaseString( fbxFileName ).ToCStr() + ".fbm/";
