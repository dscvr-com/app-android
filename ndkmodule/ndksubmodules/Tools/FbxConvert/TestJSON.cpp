...
#include "../../LibOVRKernel/Src/Kernel/OVR_SysFile.h"
#include "../../LibOVRKernel/Src/Kernel/OVR_JSON.h"
#include "../../LibOVRKernel/Src/Kernel/OVR_BinaryFile.h"

#include "File_Utils.h"

using namespace OVR;

#include "../../VrAppSupport/VrModel/Src/ModelTrace.h"
			...
			if ( joint_array.IsArray() )
			{
				while ( !joint_array.IsEndOfArray() )
				{
					...
					if ( joint.IsObject() )
					{
						...
						StringUtils::StringTo( newJoint.transform, joint.GetChildStringByName( "transform" ).ToCStr() );
			...
			if ( tag_array.IsArray() )
			{
				while ( !tag_array.IsEndOfArray() )
				{
					...
					if ( tag.IsObject() )
					{
						...
						StringUtils::StringTo( newTag.matrix, tag.GetChildStringByName( "matrix" ).ToCStr() );
						StringUtils::StringTo( newTag.jointIndices, tag.GetChildStringByName( "jointIndices" ).ToCStr() );
						StringUtils::StringTo( newTag.jointWeights, tag.GetChildStringByName( "jointWeights" ).ToCStr() );
					}
				}
			}
			...
			if ( surface_array.IsArray() )
			{
				while ( !surface_array.IsEndOfArray() )
				{
					...
					if ( surface.IsObject() )
					{
						...
						StringUtils::StringTo( newSurface.bounds, surface.GetChildStringByName( "bounds" ).ToCStr() );
						...
						if ( vertices.IsObject() )
						{
							...
							ReadModelArray( newSurface.position,     vertices.GetChildStringByName( "position" ).ToCStr(),		bin, vertexCount );
							ReadModelArray( newSurface.normal,       vertices.GetChildStringByName( "normal" ).ToCStr(),			bin, vertexCount );
							ReadModelArray( newSurface.tangent,      vertices.GetChildStringByName( "tangent" ).ToCStr(),		bin, vertexCount );
							ReadModelArray( newSurface.binormal,     vertices.GetChildStringByName( "binormal" ).ToCStr(),		bin, vertexCount );
							ReadModelArray( newSurface.color,        vertices.GetChildStringByName( "color" ).ToCStr(),			bin, vertexCount );
							ReadModelArray( newSurface.uv0,          vertices.GetChildStringByName( "uv0" ).ToCStr(),			bin, vertexCount );
							ReadModelArray( newSurface.uv1,          vertices.GetChildStringByName( "uv1" ).ToCStr(),			bin, vertexCount );
							ReadModelArray( newSurface.jointIndices, vertices.GetChildStringByName( "jointIndices" ).ToCStr(),	bin, vertexCount );
							ReadModelArray( newSurface.jointWeights, vertices.GetChildStringByName( "jointWeights" ).ToCStr(),	bin, vertexCount );
						}
						...
						if ( triangles.IsObject() )
						{
							...
							ReadModelArray( newSurface.indices, triangles.GetChildStringByName( "indices" ).ToCStr(), bin, indexCount );
						}
					}
				}
			}
		...
		if ( collision_model.IsArray() )
		{
			while ( !collision_model.IsEndOfArray() )
			{
				...
				if ( polytope.IsObject() )
				{
					...
					StringUtils::StringTo( planes, polytope.GetChildStringByName( "planes" ).ToCStr() );
				}
			}
		}
