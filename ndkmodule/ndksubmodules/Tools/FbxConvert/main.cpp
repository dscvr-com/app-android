...
#include "../../LibOVRKernel/Src/Kernel/OVR_JSON.h"
...
struct options_t
{
	...
	bool				fbxUtil;			// If set, don't pause on exit in debug mode
	bool				outputToStdOut;		// Used by FBXUtil.exe to obtain surface information
};
...
static void ShowHelp()
{
	...
	printf( "-outputToStdOut				 : Outputs information on surfaces for FBXUtil.\n" );
}
...
static void TuscanyHack( RawModel & raw )
{
	...
	{
		if ( strstr( raw.GetTexture( i ).name.ToCStr(), "Ocean.tga" ) )
		{
			diffuseTexture = i;
			break;
		}
...
static void RemoveRawModelSurfaces( RawModel & raw, const Array< const char * > & removeSurfaces )
{
	for ( int i = 0; i < raw.GetSurfaceCount(); i++ )
	{
		...
		{
			if ( WildCardICmp( raw.GetSurface( i ).name.ToCStr(), removeSurfaces[j] ) == 0 )
			{
...
static void KeepSelectedSurfaces( RawModel & raw, const Array< const char * > & keepSurfaces, const char * label )
{
	for ( int i = 0; i < raw.GetSurfaceCount(); i++ )
	{
		...
		{
			if ( WildCardICmp( raw.GetSurface( i ).name.ToCStr(), keepSurfaces[j] ) == 0 )
			{
				keep = true;
				break;
			}
									...
									const int numAtlases, const char * textureFolder, const bool repeatTextures )
{
	for ( int atlas = 0; atlas < numAtlases; atlas++ )
	{
		for ( int i = 0; i < raw.GetSurfaceCount(); i++ )
		{
			...
			{
				if ( WildCardICmp( raw.GetSurface( i ).name.ToCStr(), atlasSurfaces[atlas][j] ) == 0 )
				{
...
static void DiscretizeRawModelSurfaces( RawModel & raw, const Array< const char * > & discreteSurfaces )
{
	for ( int i = 0; i < raw.GetSurfaceCount(); i++ )
	{
		...
		{
			if ( WildCardICmp( raw.GetSurface( i ).name.ToCStr(), discreteSurfaces[j] ) == 0 )
			{
...
static void SkinRawModelSurfaces( RawModel & raw, const Array< const char * > & skinSurfaces )
{
	for ( int i = 0; i < raw.GetSurfaceCount(); i++ )
	{
		...
		{
			if ( WildCardICmp( raw.GetSurface( i ).name.ToCStr(), skinSurfaces[j] ) == 0 )
			{
...
static void TagRawModelSurfaces( RawModel & raw, const Array< const char * > & tagSurfaces )
{
	for ( int i = 0; i < raw.GetSurfaceCount(); i++ )
	{
		...
		{
			if ( WildCardICmp( raw.GetSurface( i ).name.ToCStr(), tagSurfaces[j] ) == 0 )
			{
...
bool CreateBatchFile( const char * batchFilePath, const char * exePath, const char * outputName, const RawModel & raw, const options_t & options )
{
	...
	const String exeFolder = StringUtils::GetCleanPathString( StringUtils::GetFolderString( exePath ).ToCStr(), pathSeparator );
	const String curFolder = StringUtils::GetCleanPathString( FileUtils::GetCurrentFolder().ToCStr(), pathSeparator );
	const String absFolder = ( strncmp( exeFolder.ToCStr(),  "..", 2 ) == 0 ) ? StringUtils::GetCleanPathString( String( curFolder + exeFolder ).ToCStr(), pathSeparator ) : exeFolder;
	...
	for ( int i = 0; i < raw.GetTextureCount(); i++ )
	{
		...
		const String sourceTexture = StringUtils::GetCleanPathString( texture.name.ToCStr(), pathSeparator );

		const String textureFolder = StringUtils::GetFolderString( sourceTexture.ToCStr() );
		const String textureBase = StringUtils::GetFileBaseString( sourceTexture.ToCStr() );
		const String textureExt = StringUtils::GetFileExtensionString( sourceTexture.ToCStr() );
		...
		const char * compressedExt =	( options.container == TEXTURE_CONTAINER_KTX ? ".ktx" :
										...
										".ktx" ) ) );
		const String compressedSource = StringUtils::SetFileExtensionString( sourceTexture.ToCStr(), compressedExt );

		// Check if the source texture exists.
		if ( !FileUtils::FileExists( sourceTexture.ToCStr() ) )
		{
			Warning( "WARNING: missing texture \"%s\"\n", sourceTexture.ToCStr() );
		}

		// If a compressed texture exists in the source folder.
		if ( FileUtils::FileExists( compressedSource.ToCStr() ) )
		{
		...
		{
			const String cubeMapBase = StripCubeMapSidesFileBaseExtensions( textureBase.ToCStr() );
			textureInput.Clear();
			for ( int i = 0; i < 6; i++ )
			{
				if ( i > 0 )
				...
				if ( cubeMapUpDownRotate != 0 && _stricmp( cubeMapSidesFileBaseExtensions[i], "_up" ) == 0 )
				{
					...
					RotateImage( texturePath.ToCStr(), rotatedTexturePath.ToCStr(), cubeMapUpDownRotate );
	...
	fprintf( f, "\"%s%s\" a -tzip -mx%d %s.ovrscene ./%s/*\n", absFolder.ToCStr(), zipExe, options.zip, outputName,
			StringUtils::GetRelativePathString( StringUtils::GetCleanPathString( pckFolder.ToCStr(), '/' ).ToCStr(), curFolder.ToCStr(), '/' ).ToCStr() );
...
int main( int argc, char * argv[] )
{
...
#if !defined( _DEBUG )
	const String exeFolder = StringUtils::GetCleanPathString( StringUtils::GetFolderString( argv[0] ).ToCStr(), pathSeparator );
	const String curFolder = StringUtils::GetCleanPathString( FileUtils::GetCurrentFolder().ToCStr(), pathSeparator );
	const String absFolder = ( strncmp( exeFolder.ToCStr(),  "..", 2 ) == 0 ) ? StringUtils::GetCleanPathString( String( curFolder + exeFolder ).ToCStr(), pathSeparator ) : exeFolder;
	...
	if (	!CheckFile( String( absFolder + pckExe ).ToCStr() ) ||
			!CheckFile( String( absFolder + zipExe ).ToCStr() ) )
	{
		ShowHelp();
		return -1;
	}
	...
	for ( int i = 1; i < argc; i++ )
	{
		...
		else if ( _stricmp( s, "collision" ) == 0 )
		{
			if ( i >= argc - 1 )
			{
				Error( "missing arguments for -collision\n" );
			}
			else if ( StringUtils::GetFileExtensionString( argv[i + 1] ).CompareNoCase( "fbx" ) == 0 )
			{
		...
		else if ( _stricmp( s, "ground" ) == 0 )
		{
			if ( i >= argc - 1 )
			{
				Error( "missing arguments for -ground\n" );
			}
			else if ( StringUtils::GetFileExtensionString( argv[i + 1] ).CompareNoCase( "fbx" ) == 0 )
			{
		...
		else if ( _stricmp( s, "raytrace" ) == 0 )
		{
			if ( i >= argc - 1 )
			{
				Error( "missing arguments for -raytrace\n" );
			}
			else if ( StringUtils::GetFileExtensionString( argv[i + 1] ).CompareNoCase( "fbx" ) == 0 )
			{
		...
		else if ( _stricmp( s, "attrib" ) == 0 )
		{
			if ( i >= argc - 1 )
			{
				Error( "missing arguments for -attrib\n" );
			}
			else 
			{
		...
		else if ( _stricmp( s, "expand" ) == 0 )
		{
			if ( i >= argc - 1 )
			{
				Error( "missing arguments for -expand\n" );
			}
			else
			{
				i++;
		...
		else if ( _stricmp( s, "remove" ) == 0 )
		{
			if ( i >= argc - 1 )
			{
				Error( "missing arguments for -remove\n" );
			}
			else 
			{
				for ( ; i + 1 < argc && argv[i + 1][0] != '-'; i++ )
		...
		else if ( _stricmp( s, "discrete" ) == 0 )
		{
			if ( i >= argc - 1 )
			{
				Error( "missing arguments for -discrete\n" );
			}
			else 
			{
				for ( ; i + 1 < argc && argv[i + 1][0] != '-'; i++ )
		...
		else if ( _stricmp( s, "skin" ) == 0 )
		{
			if ( i >= argc - 1 )
			{
				Error( "missing arguments for -skin\n" );
			}
			else 
			{
				for ( ; i + 1 < argc && argv[i + 1][0] != '-'; i++ )
		...
		else if ( _stricmp( s, "atlas" ) == 0 )
		{
			if ( i >= argc - 1 )
			{
				Error( "missing arguments for -atlas\n" );
			}
			else 
			{
		...
		else if ( _stricmp( s, "anim" ) == 0 )
		{
			...
			if ( i >= argc - 1 )
			{
				Error( "missing arguments for -anim\n" );
			}
			else if ( _stricmp( argv[i + 1], "rotate" ) == 0 )
			{
		...
		else if ( _stricmp( s, "include" ) == 0 )
		{
			if ( i >= argc - 1 )
			{
				Error( "missing arguments for -include\n" );
			}
			else 
			{
				for ( ; i + 1 < argc && argv[i + 1][0] != '-'; i++ )
		...
		else if ( _stricmp( s, "zip" ) == 0 )
		{
			...
			if ( i >= argc - 1 )
			{
				Error( "missing arguments for -zip\n" );
			}
			else
			{
		...
		else if ( _stricmp( s, "fbxUtil" ) == 0 )
		{
			options.fbxUtil = true;
		}
		else if ( _stricmp( s, "outputToStdOut" ) == 0 )
		{
	...
	{
		if ( !FileUtils::CreatePath( tempFolder.ToCStr() ) )
		{
	...
	if ( renderFbxFileName != NULL )
	{
		...
		if ( LoadRawModel( raw, renderFbxFileName, textureExtensions, geometryTransform, texturesTransform ) )
		{
			...
			if ( !options.outputToStdOut )
			{
				AtlasRawModelSurfaces( raw, atlasSurfaces, numAtlases, String( FileUtils::GetCurrentFolder() + tempFolder ).ToCStr(), options.repeat );
			}
			...
			if ( !options.outputToStdOut )
			{
				CreateBatchFile( batchPath.ToCStr(), argv[0], outputName, raw, options );
			}
		}
	}
	...
	{
		printf( "START_JSON\n" );
	    char* text = JSON_scene->PrintValue( 0, true );
		if ( text )
		{
			printf( "%s\n", text );
			OVR_FREE( text );
	...
	{
		printf( "writing %s\n", jsonPath.ToCStr() );
		if ( !JSON_scene->Save( jsonPath.ToCStr() ) )
		{
		...
		printf( "writing %s\n", binaryPath.ToCStr() );
		if ( !WriteBinary( binaryPath.ToCStr(),
							data_render_model,
...
#if !defined( __APPLE__ ) && !defined( __linux__ )
	ParseModelsJsonTest( jsonPath.ToCStr(), binaryPath.ToCStr() );
#endif

	printf( "-----------------------\n" );
	if ( options.pack )
	{
		FileUtils::Execute( batchPath.ToCStr() );
	}
	else
	{
