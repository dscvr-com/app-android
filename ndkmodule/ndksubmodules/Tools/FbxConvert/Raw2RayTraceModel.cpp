...
ModelData * Raw2RayTraceModel( const RawModel & raw, const bool fullText )
{
	...
	JSON * JSON_raytrace_model = JSON::CreateObject();
	{
		...
		JSON_raytrace_model->AddStringItem( "bounds", StringUtils::ToString( nodeQueue[0].aabb ).ToCStr() );
		...
		JSON_raytrace_model->AddStringItem( "vertices", fullText ? GetVector3fArrayString( vertices ).ToCStr() : "Vector3f" );
		JSON_raytrace_model->AddStringItem( "uvs", fullText ? GetVector2fArrayString( uvs ).ToCStr() : "Vector2f" );
		JSON_raytrace_model->AddStringItem( "indices", fullText ? GetInt32ArrayString( indices ).ToCStr() : "Int32" );
		...
		if ( fullText )
		{
			...
			for ( int i = 0; i < tree_leafs.GetSizeI(); i++ )
			{
				...
				JSON_leaf->AddStringItem( "triangles", StringUtils::ToString( tree_leafs[i].triangles, RT_KDTREE_MAX_LEAF_TRIANGLES ).ToCStr() );
				JSON_leaf->AddStringItem( "ropes", StringUtils::ToString( tree_leafs[i].ropes, 6 ).ToCStr() );
				JSON_leaf->AddStringItem( "bounds", StringUtils::ToString( tree_leafs[i].bounds ).ToCStr() );
		...
		JSON_raytrace_model->AddStringItem( "overflow", fullText ? GetInt32ArrayString( tree_overflow ).ToCStr() : "Int32" );
	}
