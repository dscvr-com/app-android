...
#define FBX_TOOL
#include "../../LibOVRKernel/Src/Kernel/OVR_Hash.h"
#include "../../LibOVRKernel/Src/Kernel/OVR_String.h"
#include "../../LibOVRKernel/Src/Kernel/OVR_String_Utils.h"
...
void RawModel::Condense()
{
	// Only keep surfaces that are referenced by one or more triangles.
	{
		...
		for ( int i = 0; i < triangles.GetSizeI(); i++ )
		{
			...
			const int surfaceIndex = AddSurface( surface.name.ToCStr(), surface.transform );
		...
		for ( int i = 0; i < materials.GetSizeI(); i++ )
		{
			for ( int j = 0; j < RAW_TEXTURE_USAGE_MAX; j++ )
			{
				if ( materials[i].textures[j] >= 0 )
				{
					...
					const int textureIndex = AddTexture( texture.name.ToCStr(), texture.usage );
...
void RawModel::StripModoNumbers()
{
	for ( int i = 0; i < surfaces.GetSizeI(); i++ )
	{
		const String newName = StripNumbers( surfaces[i].name.ToCStr() );
	...
	for ( int i = 0; i < unique.GetSizeI(); i++ )
	{
		...
		if ( unique[i].repeatSurfaceIndex != -1 )
		{
			Warning( "single triangle from surface '%s' repeats texture '%s': %1.0f, %1.0f\n",
						...
						StringUtils::GetFileNameString( textures[mat.textures[0]].name.ToCStr() ).ToCStr(),
		...
		for ( int j = 0; j < RAW_TEXTURE_USAGE_MAX; j++ )
		{
			...
			printf( "creating %dx%d '%s' (repeat %d,%d)\n", newWidth, newHeight,
					StringUtils::GetFileNameString( textures[mat.textures[j]].name.ToCStr() ).ToCStr(), repeatX, repeatY );
			...
			const String uniqueFileName = (const char *)StringUtils::Va( "%s%s_%dx%d_%s.tga",
							textureFolder, StringUtils::GetFileBaseString( textures[mat.textures[j]].name.ToCStr() ).ToCStr(),
							newWidth, newHeight, usageStr );
			if ( stbi_write_tga( uniqueFileName.ToCStr(), newWidth, newHeight, 4, newData ) == 0 )
			{
			...
			newMaterialTextures[j] = AddTexture( uniqueFileName.ToCStr(), (RawTextureUsage) j );
		}
...
bool RawModel::CreateTextureAtlases( const char * textureFolder, const bool repeatTextures )
{
	...
	for ( int atlas = 0; atlas < numAtlases; atlas++ )
	{
							...
							am.minUv0.y < 0.0f - uvEpsilon || am.minUv0.y > 1.0f + uvEpsilon )
					{
						for ( int n = 0; n < am.surfaces.GetSizeI(); n++ )
						{
							Warning( "surface '%s' tiles '%s' over [%3.1f,%3.1f]-[%3.1f,%3.1f]\n",
										...
										StringUtils::GetFileNameString( textures[mat.textures[usage]].name.ToCStr() ).ToCStr(),
		...
		for ( int usage = 0; usage < RAW_TEXTURE_USAGE_MAX; usage++ )
		{
			...
			{
				atlasTextures[atlas][usage][i].data = stbi_load( atlasTextures[atlas][usage][i].name.ToCStr(),
				...
				{
					Error( "failed to load '%s'\n", (const char *)atlasTextures[atlas][usage][i].name.ToCStr() );
					return false;
				}
			...
			if ( stbi_write_tga( atlasFileName.ToCStr(), totalWidth, totalHeight, components, atlasData ) == 0 )
			{
			...
			newAtlasTextures[usage] = AddTexture( atlasFileName.ToCStr(), (RawTextureUsage) usage );
...
void RawModel::SkinRigidSurfaces()
{
	...
	for ( int i = 0; i < triangles.GetSizeI(); i++ )
	{
		for ( int j = 0; j < 3; j++ )
		{
			...
			{
				vertex.jointIndices[0] = AddJoint( surfaces[triangles[i].surfaceIndex].name.ToCStr(), surfaces[triangles[i].surfaceIndex].transform );
...
void RawModel::SortGeometry( const int axis )
{
	...
	switch ( axis )
	{
		case SORT_ORIGIN_NEG:	Alg::QuickSort( vertices, VertexOriginSortNeg::Compare ); break;			// back-to-front from origin
		case SORT_AXIS_NEG_Z:	Alg::QuickSort( vertices, VertexAxisSortNegative< 2 >::Compare ); break;	// back-to-front looking down negative Z
		case SORT_AXIS_NEG_Y:	Alg::QuickSort( vertices, VertexAxisSortNegative< 1 >::Compare ); break;	// back-to-front looking down negative Y
		case SORT_AXIS_NEG_X:	Alg::QuickSort( vertices, VertexAxisSortNegative< 0 >::Compare ); break;	// back-to-front looking down negative X
		case SORT_AXIS_POS_X:	Alg::QuickSort( vertices, VertexAxisSortPositive< 0 >::Compare ); break;	// front-to-back looking down negative X
		case SORT_AXIS_POS_Y:	Alg::QuickSort( vertices, VertexAxisSortPositive< 1 >::Compare ); break;	// front-to-back looking down negative Y
		case SORT_AXIS_POS_Z:	Alg::QuickSort( vertices, VertexAxisSortPositive< 2 >::Compare ); break;	// front-to-back looking down negative Z
		case SORT_ORIGIN_POS:	Alg::QuickSort( vertices, VertexOriginSortPos::Compare ); break;			// front-to-back from origin
		default: return;
	}
	...
	if ( invertedTransparencySort )
	{
		...
		Array< RawTriangle > opaqueTriangles;
		Array< RawTriangle > transparentTriangles;
		for ( int i = 0; i < triangles.GetSizeI(); i++ )
		{
			const int materialIndex = triangles[i].materialIndex;
			if ( materialIndex < 0 )
			{
				opaqueTriangles.PushBack( triangles[i] );
				continue;
			}
			...
			{
				transparentTriangles.PushBack( triangles[i] );
			}
			else
			{
				opaqueTriangles.PushBack( triangles[i] );
		...
		// Sort the opaque triangles based on the first vertex index of each triangle.
		Alg::QuickSort( opaqueTriangles, TriangleVertexSortPos::Compare );

		// Sort the transparent triangles in the opposite direction based on the first vertex index of each triangle.
		Alg::QuickSort( transparentTriangles, TriangleVertexSortNeg::Compare );

		// Add the triangles back to the main list.
		triangles.Clear();
		for ( int i = 0; i < opaqueTriangles.GetSizeI(); i++ )
		{
			triangles.PushBack( opaqueTriangles[i] );
		}
		for ( int i = 0; i < transparentTriangles.GetSizeI(); i++ )
		{
			triangles.PushBack( transparentTriangles[i] );
		}
	}
	else
	{
		// Sort the triangles based on the first vertex index of each triangle.
		Alg::QuickSort( triangles, TriangleVertexSortPos::Compare );
	}
...
void RawModel::CreateMaterialModels( Array< RawModel > & materialModels, const int maxModelVertices, const int keepAttribs ) const
{
	...
	Array< RawTriangle > sortedTriangles;
	...
	if ( invertedTransparencySort )
	{
		...
		Alg::QuickSort( opaqueTriangles, TriangleModelSortPos::Compare );

		// Sort the transparent triangles in the opposite direction.
		Alg::QuickSort( transparentTriangles, TriangleModelSortNeg::Compare );

		// Add the triangles to the sorted list.
		for ( int i = 0; i < opaqueTriangles.GetSizeI(); i++ )
		{
			sortedTriangles.PushBack( opaqueTriangles[i] );
		}
		for ( int i = 0; i < transparentTriangles.GetSizeI(); i++ )
		{
			sortedTriangles.PushBack( transparentTriangles[i] );
		}
	}
	else
	{
		sortedTriangles = triangles;
		Alg::QuickSort( sortedTriangles, TriangleModelSortPos::Compare );
	}
	...
	for ( int i = 0; i < sortedTriangles.GetSizeI(); i++ )
	{
		...
		const int surfaceIndex = model->AddSurface( surfaces[sortedTriangles[i].surfaceIndex].name.ToCStr(), surfaces[sortedTriangles[i].surfaceIndex].transform );
