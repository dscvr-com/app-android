...
bool CompileShader( GLenum shaderType, const GLuint shader, const char * src )
{
#if defined( OVR_OS_ANDROID )
	...
	const char * srcs[2];
	srcs[0] = shaderType == GL_VERTEX_SHADER ? vertexShaderHeader : fragmentShaderHeader;
	srcs[1] = src;
#endif
...
GlProgram BuildProgram( const char * vertexSrc,
		const char * fragmentSrc, bool abortOnError )
{
	GlProgram prog;

	prog.vertexShader = glCreateShader( GL_VERTEX_SHADER );
	if ( !CompileShader( GL_VERTEX_SHADER, prog.vertexShader, vertexSrc ) )
	{
		if ( abortOnError )
		{
	...
	if ( !CompileShader( GL_FRAGMENT_SHADER, prog.fragmentShader, fragmentSrc ) )
	{
		if ( abortOnError )
		{
	...
	prog.uClipUVs = glGetUniformLocation( prog.program, "ClipUVs" );
