...
static bool GlWindow_ProcessEvents( GlWindow_t * window )
{
	OVR_UNUSED( window );
...
glSetup_t GL_Setup( const int windowWidth, const int windowHeight, const bool fullscreen, AppLocal * appLocal )
{
	glSetup_t gl = {};
...
bool GL_ProcessEvents()
{
	bool exit = false;
	//const Microseconds_t time = GetTimeMicroseconds();

	if ( GlWindow_ProcessEvents( &glWindow ) )
	{
		exit = true;
	}

	if ( GlWindow_CheckKeyboardKey( &glWindow, OVR_KEY_ESCAPE ) )
	{
		exit = true;
	}
