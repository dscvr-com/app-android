...
SoundAssetMapping is a simple sound asset management class which allows sound assets to be easily replaced without recompilation. 
SoundAssetMapping is driven by a JSON file in which sounds are mapped as key-value pairs, where a value is the actual path to the wav file. 
...
For development, SoundAssetMapping checks for a sound_assets.json within the Oculus folder either on the internal or external
