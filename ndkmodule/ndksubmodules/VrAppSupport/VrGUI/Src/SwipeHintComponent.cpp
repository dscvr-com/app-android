...
namespace OVR
{
	...
	eMsgStatus OvrSwipeHintComponent::OnEvent_Impl( OvrGuiSys & guiSys, VrFrame const & vrFrame,
	...
	eMsgStatus OvrSwipeHintComponent::Opening( OvrGuiSys & guiSys, VrFrame const & vrFrame, VRMenuObject * self, VRMenuEvent const & event )
	{
		Reset( self );
		return MSG_STATUS_ALIVE;
	}
	...
	eMsgStatus OvrSwipeHintComponent::Frame( OvrGuiSys & guiSys, VrFrame const & vrFrame, VRMenuObject * self, VRMenuEvent const & event )
	{
		...
		if ( alpha > 0.0f )
		{
			...
			else
			{
				...
				alpha *= sin( Mathf::Pi * 2.0f * normTime );
