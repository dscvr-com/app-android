...
void UIObject::AlignTo( const RectPosition myPosition, const UIObject *other, const RectPosition otherPosition, const float zOffset )
{
	...
	const Vector3f alignPosWorld = otherWorldPose.Orientation.Rotate( alignPos * DEFAULT_TEXEL_SCALE * otherScale ) + otherWorldPose.Position;

	// convert align position from world space to parent space
	Vector3f alignPosLocal = parentWorldPose.Orientation.Inverted().Rotate( alignPosWorld - parentWorldPose.Position );
	...
	Posef myLocalPose;
	myLocalPose.Orientation = parentWorldPose.Orientation.Inverted() * otherWorldPose.Orientation;
	...
	myLocalPose.Position = alignPosLocal - ( myAlignPos * DEFAULT_TEXEL_SCALE * GetLocalScale().x );
	myLocalPose.Position.z += zOffset;
...
void UIObject::RemoveComponent( VRMenuComponent * component )
{
	VRMenuObject * object = GetMenuObject();
	OVR_ASSERT( object );
	if ( object != NULL )
...
void UIObject::WrapChildrenHorizontal()
{
	...
	for( UPInt i = 0; i < children.GetSize(); i++ )
	{
		if ( children[ i ]->GetVisible() )
		{
			...
			const float 	objectWidth = paddedRect.GetSize().Width * DEFAULT_TEXEL_SCALE * children[ i ]->GetLocalScale().x;
