...
{
	// GuiSys.GetVRMenuMgr().SetShowDebugBounds(true);
	...
	if ( parent != NULL )
	{
		barWidth = parent->GetMarginRect().GetWidth() - 18.0f - ( showDescriptionLabel ? 0.0f : showCancelButton ? 40.0 : 0.0f );
	...
	ProgressBarTexture.LoadTextureFromApplicationPackage( "assets/img_progressbar_foreground.png" );
	ProgressBarBackgroundTexture.LoadTextureFromApplicationPackage( "assets/img_progressbar_background.png" );
	...
	if ( showCancelButton )
	{
		CancelButtonTexture.LoadTextureFromApplicationPackage( "assets/img_btn_download_cancel.png" );
