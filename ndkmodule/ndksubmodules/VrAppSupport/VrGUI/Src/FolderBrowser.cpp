...
private:
	...
	eMsgStatus Frame( OvrGuiSys & guiSys, VrFrame const & vrFrame, VRMenuObject * self, VRMenuEvent const & event )
	{
		...
		{
			float timeDiff = vrapi_GetTimeInSeconds() - LastInteractionTimeStamp;
	...
	eMsgStatus TouchRelative( OvrGuiSys & guiSys, VrFrame const & vrFrame, VRMenuObject * self, VRMenuEvent const & event )
	{
		if ( FolderBrowser.HasNoMedia() )
		{
			return MSG_STATUS_CONSUMED;
	...
	eMsgStatus OnOpening( OvrGuiSys & guiSys, VrFrame const & vrFrame, VRMenuObject * self, VRMenuEvent const & event )
	{
		return MSG_STATUS_ALIVE;
	}
...
private:
    ...
    virtual eMsgStatus Frame( OvrGuiSys & guiSys, VrFrame const & vrFrame, VRMenuObject * self, VRMenuEvent const & event )
    {
    	OVR_ASSERT( FolderPtr );
    ...
    eMsgStatus OnEvent_Impl( OvrGuiSys & guiSys, VrFrame const & vrFrame, VRMenuObject * self, VRMenuEvent const & event )
    {
    	switch( event.EventType )
    	{
    		case VRMENU_EVENT_FRAME_UPDATE:
	...
	eMsgStatus OnTouchDown_Impl( OvrGuiSys & guiSys, VrFrame const & vrFrame, VRMenuObject * self, VRMenuEvent const & event )
	{
		ScrollMgr.TouchDown();
		TouchDown = true;
		return MSG_STATUS_ALIVE;
	...
	eMsgStatus OnTouchUp_Impl( OvrGuiSys & guiSys, VrFrame const & vrFrame, VRMenuObject * self, VRMenuEvent const & event )
	{
		ScrollMgr.TouchUp();
		TouchDown = false;
		return MSG_STATUS_ALIVE;
...
void OvrFolderBrowser::Frame_Impl( OvrGuiSys & guiSys, VrFrame const & vrFrame )
{
	...
	else
	{
		if( rightPressed || leftPressed )
		{
			...
			else
			{
				...
				LastControllerInputTimeStamp = vrapi_GetTimeInSeconds();
			}
		}
...
void OvrFolderBrowser::OneTimeInit( OvrGuiSys & guiSys )
{
	...
	menuHandle_t scrollDownHintHandle = OvrSwipeHintComponent::CreateSwipeSuggestionIndicator( guiSys, this, ScrollSuggestionRootHandle, suggestionDownId.Get(),
	...
	menuHandle_t scrollUpHintHandle = OvrSwipeHintComponent::CreateSwipeSuggestionIndicator( guiSys, this, ScrollSuggestionRootHandle, suggestionUpId.Get(),
