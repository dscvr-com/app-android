...
void ResumeMovieView::OneTimeInit( const char * launchIntent )
{
	...
	LOG( "ResumeMovieView::OneTimeInit: %3.1f seconds", vrapi_GetTimeInSeconds() - start );
}
...
Matrix4f ResumeMovieView::GetEyeViewMatrix( const int eye ) const
{
	return Cinema.SceneMgr.GetEyeViewMatrix( eye );
}

Matrix4f ResumeMovieView::GetEyeProjectionMatrix( const int eye, const float fovDegreesX, const float fovDegreesY ) const 
{
	return Cinema.SceneMgr.GetEyeProjectionMatrix( eye, fovDegreesX, fovDegreesY );
}

Matrix4f ResumeMovieView::DrawEyeView( const int eye, const float fovDegreesX, const float fovDegreesY, ovrFrameParms & frameParms )
{
