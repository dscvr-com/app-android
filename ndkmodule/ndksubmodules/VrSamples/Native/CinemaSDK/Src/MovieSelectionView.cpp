...
void MovieSelectionView::SetCategory( const MovieCategory category )
{
	...
	for( int i = 0; i < NumSwipeTrails; i++ )
	{
		CarouselSwipeHintComponent * compLeft = LeftSwipes[ i ]->GetMenuObject()->GetComponentByName<CarouselSwipeHintComponent>();
		compLeft->Reset( LeftSwipes[ i ]->GetMenuObject() );
		CarouselSwipeHintComponent * compRight = RightSwipes[ i ]->GetMenuObject()->GetComponentByName<CarouselSwipeHintComponent>();
...
Matrix4f MovieSelectionView::GetEyeViewMatrix( const int eye ) const
{
	return Cinema.SceneMgr.GetEyeViewMatrix( eye );
}

Matrix4f MovieSelectionView::GetEyeProjectionMatrix( const int eye, const float fovDegreesX, const float fovDegreesY ) const 
{
	return Cinema.SceneMgr.GetEyeProjectionMatrix( eye, fovDegreesX, fovDegreesY );
}

Matrix4f MovieSelectionView::DrawEyeView( const int eye, const float fovDegreesX, const float fovDegreesY, ovrFrameParms & frameParms )
{
	return Cinema.SceneMgr.DrawEyeView( eye, fovDegreesX, fovDegreesY, frameParms );
}
