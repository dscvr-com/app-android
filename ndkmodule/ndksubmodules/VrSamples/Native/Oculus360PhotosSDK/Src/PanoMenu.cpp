...
private:
	...
	eMsgStatus OnFrame( OvrGuiSys & guiSys, VrFrame const & vrFrame, VRMenuObject * self, VRMenuEvent const & event )
	{
		OVR_UNUSED( vrFrame );
		OVR_UNUSED( event );
...
void OvrPanoMenu::Frame_Impl( OvrGuiSys & guiSys, VrFrame const & vrFrame )
{
...
void OvrPanoMenu::OnItemEvent_Impl( OvrGuiSys & guiSys, VRMenuId_t const itemId, VRMenuEvent const & event )
{
	OVR_UNUSED( event );
...
bool OvrPanoMenu::GazingAtMenu( const Matrix4f & viewMatrix ) const
{
	...
	const float dot = viewForward.Dot( FWD * GetMenuPose().Orientation );
	if ( dot >= cosAngle )
	{
		return true;
	}
