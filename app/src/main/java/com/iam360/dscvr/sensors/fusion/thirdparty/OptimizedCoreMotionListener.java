package com.iam360.dscvr.sensors.fusion.thirdparty;
...
/**
 * Implementation of Alexander Pacha's optimized core motion listeners, fused to DSCVRs internal
 * Rotation Matrix Source interface.
 *
 ...
 *
 * @author Alexander Pacha, Emanuel Jöbstl
 *
 */

public class OptimizedCoreMotionListener extends CoreMotionListener {

    public OptimizedCoreMotionListener(Context context) {
        super(context);
    }

    @Override
    public int[] getRequiredSensorTypes() {
        int[] base = super.getRequiredSensorTypes();
        int[] types = new int[base.length + 1];
        System.arraycopy(base, 0, types, 0, base.length);
        types[base.length] = Sensor.TYPE_GYROSCOPE;
        return types;
    }
     ...
     */
    private synchronized void setOrientationQuaternionAndMatrix(Quaternion quaternion) {
