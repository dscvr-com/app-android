 ...
 */
public class Cube {
    private static final int PLANES_PER_CUBE = 1;
    ...
    // POSITIONS
    private static final float[] POSITION_AHEAD = {0, 0, 0};
    ...
    private Plane face_ahead;
    private Plane[] planes = {face_ahead};
    ...
    private void initializeTransforms() {
        float[] transformAhead = computeTransform(POSITION_AHEAD, ROTATION_AHEAD, SIDE_LENGTH);
        transforms.add(transformAhead);

        Log.d(Constants.DEBUG_TAG, Arrays.toString(transformAhead));
    ...
    private float[] computeTransform(float[] translation, float[] rotation, float scale) {
        ...
        Matrix.translateM(trsMatrix, 0, rsMatrix, 0, translation[0], translation[1], translation[2]);
